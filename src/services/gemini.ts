...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemine.models.generateContent({
    model,
    contents: [
      {
  ...
  if (!response.text) {
    throw new Error("Wasn't able to transcribe audio.")
  }
...
export async function generateEmbeddings(text: string) {
  const response = await gemine.models.embedContent({
    model: 'text-embedding-004',
    contents: [{ text }],
    config: {
      taskType: 'RETRIEVAL_DOCUMENT', // para qual finalidade os nossos embeddings serão usados (em nosso caso será usado para busca de similaridade/semântica)
    },
  })

  if (!response.embeddings?.[0].values) {
    throw new Error("Wasn't able to generate embeddings.")
  }
...
export async function generateAnswer(
  ...
  if (!response.text) {
    throw new Error("Wasn't able to generate answer.")
  }
